...
var Typeling = Backbone.Model.extend({
	...
	generate: function() {
		...
		for(var i = 0; i < 4; i++){
			...
			self.styles[i] = self.svg.style('.' + id + '-color-' + i + ' { fill: ' + randHex() + ' }', {id: id + '-color-' + i});
		}
	...
	mutate: function(){
		...
		switch(rand(0,9) ){
			...
			case 5:
				...
				$(g).attr('class', id + '-color-' + rand(0,3));
			...
			case 8:
				...
				var style = svg.getElementById(id + "-color-" + i);	
			...
			default:
				...
				svg.path(g, randFrom(chars).d, {class: id + '-color-' + rand(0,3), stroke: 'none'});
